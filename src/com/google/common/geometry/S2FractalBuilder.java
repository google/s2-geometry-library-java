 ...
 * corresponds to the standard Koch snowflake.  (The west coast of Britain has a fractal dimension
 * of approximately 1.25)
 * 
 * <p>The fractal is obtained by starting with an equilateral triangle and recursively subdividing
 * each edge into four segments of equal length.  Therefore the shape at level 'n' consists of
 * 3 * (4^n) edges.  Multi-level fractals are also supported: if you set minLevel() to a
 * non-negative value, then the recursive subdivision has an equal probability of stopping at any of
 * the levels between the given min and max (inclusive).  This yields a fractal where the perimeter
 * of the original triangle is approximately equally divided between fractals at the various
 * possible levels.  If there are k distinct levels {min, ..., max}, the expected number of edges
 * at each level 'i' is approximately 3 * (4 ^ i) / k.
 */
...
class S2FractalBuilder {
   ...
   * the min and max levels to be the same.  A minLevel of 0 should be avoided since this creates
   * a significant chance that none of the three original edges will be subdivided at all.
   */
  ...
  /**
   * Sets the fractal dimension.  The default value of approximately 1.26 corresponds to the
   * standard Koch curve.  The value must lie in the range [1.0, 2.0).
   */
   ...
   */
  @GwtIncompatible(value = "roundToInt is not GWT Compatible")
  public void setLevelForApproxMinEdges(int minEdges) {
    // Map values in the range [1.5 * (4 ^ n), 6 * (4 ^ n)] to level n. 
    setMinLevel(DoubleMath.roundToInt(0.5 * DoubleMath.log2(minEdges / 3), RoundingMode.UP));
  }

  @GwtIncompatible(value = "roundToInt is not GWT Compatible")
  public void setLevelForApproxMaxEdges(int maxEdges) {
    // Map values in the range [1.5 * (4 ^ n), 6 * (4 ^ n)] to level n.
    setMaxLevel(DoubleMath.roundToInt(0.5 * DoubleMath.log2(maxEdges / 3), RoundingMode.UP));
  }
  ...
  /** 
   * Given the two endpoints (v0, v4) of an edge, recursively subdivide the edge to the
   * desired level, and insert all vertices of the resulting curve up to but not including the
   * endpoint "v4".
   */
  private void getR2VerticesHelper(R2Vector v0, R2Vector v4, int level, List<R2Vector> vertices) {
    ...
    R2Vector v2 = R2Vector.sub(R2Vector.mul(R2Vector.add(v0, v4), 0.5),
        R2Vector.mul(dir.ortho(), offsetFraction));
