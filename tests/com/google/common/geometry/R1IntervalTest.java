...
public strictfp class R1IntervalTest extends GeometryTestCase {
  ...
  public void testBasics() {
    ...
    assertEquals(0d, unit.lo());
    assertEquals(1d, unit.hi());
    assertEquals(-1d, negunit.getValue(Endpoint.LO));
    assertEquals(-1d, negunit.getValue(Endpoint.LO));
    assertEquals(0d, negunit.getValue(Endpoint.HI));
    ...
    // getCenter(), getLength()
    assertEquals(0.5, unit.getCenter());
    assertEquals(0.5, half.getCenter());
    assertEquals(negunit.getLength(), 1.0);
    assertEquals(half.getLength(), 0d);
    ...
    R1Interval r = empty;
    r = r.addPoint(5);
    assertEquals(5d, r.lo());
    assertEquals(5d, r.hi());
    r = r.addPoint(-1);
    assertEquals(-1d, r.lo());
    assertEquals(5d, r.hi());
    r = r.addPoint(0);
    assertEquals(-1d, r.lo());
    assertEquals(5d, r.hi());
    ...
    // clampPoint()
    assertEquals(0.3, new R1Interval(0.1, 0.4).clampPoint(0.3));
    assertEquals(0.1, new R1Interval(0.1, 0.4).clampPoint(-7.0));
    assertEquals(0.4, new R1Interval(0.1, 0.4).clampPoint(0.6));
  ...
  public void testApproxEquals() {
    ...
    // The kLo bound is a bit closer to epsilon in Java compared to C++, due to the use of strictfp.
