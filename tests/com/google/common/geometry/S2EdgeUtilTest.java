...
public strictfp class S2EdgeUtilTest extends GeometryTestCase {
  ...
  public void testCrossings() {
    ...
    checkCrossings(
        new S2Point(1, 1, 1),
        new S2Point(1, nextAfter(1d, 0), -1),
        new S2Point(11, -12, -1),
        new S2Point(10, 10, 1),
        1,
        true,
    ...
    checkCrossings(
        new S2Point(1, 1, 1),
        new S2Point(1, nextAfter(1d, 2), -1),
        new S2Point(1, -1, 0),
        new S2Point(1, 1, 0),
        -1,
        false,
    ...
    checkCrossings(
        new S2Point(0, 0, 1),
        new S2Point(2, 1e-323, 1),
        new S2Point(1, -1, 1),
        new S2Point(1e-323, 0, 1),
        -1,
        false,
        false);
    ...
    checkCrossings(
        new S2Point(1, -1e-323, -1e-323),
        new S2Point(1e-323, 1, 1e-323),
        new S2Point(1, -1, 1e-323),
        new S2Point(1, 1, 0),
        1,
        true,
        false);
  ...
  public void testMaxLatitudeSimple() {
    ...
    assertTrue(
        getEdgeBound(1, 1, 1, 1, -1, -1)
            .approxEquals(
                new S2LatLngRect(
                    new R1Interval(-kCubeLat, kCubeLat), new S1Interval(-S2.M_PI_4, S2.M_PI_4)),
                RECT_ERROR));
    assertTrue(
        getEdgeBound(1, -1, 1, 1, 1, -1)
            .approxEquals(
    ...
    assertDoubleNear(
        S2.M_PI_4 + 0.5 * RECT_ERROR.lat().radians(), getEdgeBound(1, 1, 1, 1, -1, 1).lat().hi());
    // Max latitude, CCW edge
    assertDoubleNear(
        S2.M_PI_4 + 0.5 * RECT_ERROR.lat().radians(), getEdgeBound(1, -1, 1, 1, 1, 1).lat().hi());
    // Min latitude, CW edge
    assertDoubleNear(
        -S2.M_PI_4 - 0.5 * RECT_ERROR.lat().radians(),
    ...
    assertDoubleNear(S2.M_PI_2, getEdgeBound(.3, .4, 1, -.3, -.4, 1).lat().hi());
    assertDoubleNear(-S2.M_PI_2, getEdgeBound(.3, .4, -1, -.3, -.4, -1).lat().lo());
  }
  ...
  public void testMaxLatitudeRandom() {
    ...
    for (int iter = 0; iter < kIters; ++iter) {
      ...
      u =
          S2Point.normalize(
              new S2Point(u.x, u.y, S2.DBL_EPSILON * 1e-6 * Math.pow(1e12, rand.nextDouble())));
  ...
  private S2Point perturbATowardsB(S2Point a, S2Point b) {
    ...
    if (choice < 0.3) {
      ...
      for (; ; ) {
        S2Point temp =
            S2Point.mul(a, 2 - a.norm() + 5 * (rand.nextDouble() - 0.5) * S2.DBL_EPSILON);
  ...
  private S2Point pointNearEquator() {
    return perturbATowardsB(
        S2Point.normalize(new S2Point(rand.nextDouble(), rand.nextDouble(), 0)), randomPole());
  }
  ...
  public void testExpandForSubregions() {
    ...
    // (S2.M_PI_2 - 2e-16).
    assertTrue(getSubregionBound(-S2.M_PI_2, -1e-15, S2.M_PI_2 - 7e-16, 0).isFull());
    assertFalse(getSubregionBound(-S2.M_PI_2, -1e-15, S2.M_PI_2 - 30e-16, 0).isFull());
    assertTrue(getSubregionBound(-S2.M_PI_2 + 4e-16, 0, S2.M_PI_2 - 2e-16, 1e-7).isFull());
    assertFalse(getSubregionBound(-S2.M_PI_2 + 30e-16, 0, S2.M_PI_2, 1e-7).isFull());
    assertTrue(getSubregionBound(-S2.M_PI_2 + 4e-16, 0, S2.M_PI_2 - 4e-16, S2.M_PI_2).isFull());
    assertFalse(getSubregionBound(-S2.M_PI_2, 0, S2.M_PI_2 - 30e-16, S2.M_PI_2).isFull());
    ...
    // apart in latitude.
    assertTrue(
        getSubregionBound(1.5, -S2.M_PI_2, 1.5, S2.M_PI_2 - 2e-16)
            .approxEquals(
                new S2LatLngRect(new R1Interval(1.5, 1.5), S1Interval.full()), RECT_ERROR));
    assertTrue(
        getSubregionBound(1.5, -S2.M_PI_2, 1.5, S2.M_PI_2 - 7e-16)
            .approxEquals(
                new S2LatLngRect(
                    new R1Interval(1.5, 1.5), new S1Interval(-S2.M_PI_2, S2.M_PI_2 - 7e-16)),
                RECT_ERROR));
    ...
    assertTrue(
        getSubregionBound(-S2.M_PI_2 + 1e-15, 0, -S2.M_PI_2 + 1e-15, 0)
            .approxEquals(
  ...
  public void testLongitudePruner() {
    LongitudePruner pruner1 =
        new LongitudePruner(new S1Interval(0.75 * S2.M_PI, -0.75 * S2.M_PI), new S2Point(0, 1, 2));
    ...
    LongitudePruner pruner2 =
        new LongitudePruner(new S1Interval(0.25 * S2.M_PI, 0.25 * S2.M_PI), new S2Point(1, 0, 0));
  ...
  public void testUpdateMaxDistance() {
    checkMaxDistance(new S2Point(1, 0, 1), S2Point.X_POS, S2Point.Y_POS, M_PI_2);
    checkMaxDistance(new S2Point(1, 0, -1), S2Point.X_POS, S2Point.Y_POS, M_PI_2);
    checkMaxDistance(new S2Point(0, 1, 1), S2Point.X_POS, S2Point.Y_POS, M_PI_2);
    checkMaxDistance(new S2Point(0, 1, -1), S2Point.X_POS, S2Point.Y_POS, M_PI_2);

    double expectedRadians = Math.asin(Math.sqrt(2. / 3));
    checkMaxDistance(new S2Point(1, 1, 1), S2Point.X_POS, S2Point.Y_POS, expectedRadians);
    checkMaxDistance(new S2Point(1, 1, -1), S2Point.X_POS, S2Point.Y_POS, expectedRadians);

    checkMaxDistance(S2Point.X_POS, new S2Point(1, 1, 0), new S2Point(1, -1, 0), M_PI_4);
    checkMaxDistance(S2Point.Y_POS, new S2Point(1, 1, 0), new S2Point(-1, 1, 0), M_PI_4);
    checkMaxDistance(S2Point.Z_POS, new S2Point(0, 1, 1), new S2Point(0, -1, 1), M_PI_4);

    checkMaxDistance(S2Point.Z_POS, S2Point.X_POS, new S2Point(1, 0, -1), 3 * M_PI_4);
    checkMaxDistance(S2Point.Z_POS, S2Point.X_POS, new S2Point(1, 1, -Math.sqrt(2)), 3 * M_PI_4);

    checkMaxDistance(S2Point.Z_POS, S2Point.Z_NEG, S2Point.Z_NEG, Math.PI);
  }
  ...
  private void checkEdgePairDistance(
      ...
      S2Point expectedB) {
    a0 = S2Point.normalize(a0);
    a1 = S2Point.normalize(a1);
    b0 = S2Point.normalize(b0);
    b1 = S2Point.normalize(b1);
    expectedA = S2Point.normalize(expectedA);
    expectedB = S2Point.normalize(expectedB);
    S2Point[] result = new S2Point[2];
    S2EdgeUtil.getEdgePairClosestPoints(a0, a1, b0, b1, result);
    if (result[0].equalsPoint(S2Point.ORIGIN)) {
    ...
    if (expectedB.equalsPoint(S2Point.ORIGIN)) {
  ...
  private static boolean isEdgeBNearEdgeA(
    ...
    S2Point furthest =
        S2Point.normalize(S2Point.sub(aOrtho, S2Point.mul(bOrtho, aOrtho.dotProd(bOrtho))));
  ...
  @GwtIncompatible("fails due to inadequate robustness in GWT's S2Predicates.sign method")
  public void testCoincidentZeroLengthEdgesThatDontTouch() {
    ...
    // by sign().  It would be better to replace this with a test that
  ...
  private S2Point perturbedCornerOrMidpoint(S2Point p, S2Point q) {
    S2Point a =
        S2Point.add(S2Point.mul(p, rand.nextInt(3) - 1), S2Point.mul(q, rand.nextInt(3) - 1));
    if (oneIn(10)) {
  ...
  private double unorderedUniform(double a, double b) {
    return (a < b) ? uniform(a, b) : uniform(b, a);
  }
  ...
  private double chooseEndpoint(R1Interval clip) {
    if (oneIn(5)) {
      ...
      switch (rand.nextInt(3)) {
        case 0:
          return clip.lo() - rand.nextDouble();
        case 1:
          return clip.hi() + rand.nextDouble();
        default:
          return clip.lo() + rand.nextDouble() * clip.getLength();
      }
    }
  }
  ...
  private void checkCrossingEdges(CrossingEdgesVisitor visitor) {
    for (int i = 0; i < 5000; ++i) {
      ...
      double slope = 1e-15 * Math.pow(1e30, rand.nextDouble());
      d2 = S2Point.add(d1, S2Point.mul(d2, slope)).normalize();
      ...
      S2Point d;
      do {
        double abLen = Math.pow(1e-15, rand.nextDouble());
        double cdLen = Math.pow(1e-15, rand.nextDouble());
        double aFraction = Math.pow(1e-5, rand.nextDouble());
        aFraction = rand.nextBoolean() ? aFraction : 1 - aFraction;
        double cFraction = Math.pow(1e-5, rand.nextDouble());
        cFraction = rand.nextBoolean() ? cFraction : 1 - cFraction;
        a = S2Point.normalize(S2Point.sub(p, S2Point.mul(d1, aFraction * abLen)));
        b = S2Point.normalize(S2Point.add(p, S2Point.mul(d1, (1 - aFraction) * abLen)));
        c = S2Point.normalize(S2Point.sub(p, S2Point.mul(d2, cFraction * cdLen)));
        d = S2Point.normalize(S2Point.add(p, S2Point.mul(d2, (1 - cFraction) * cdLen)));
