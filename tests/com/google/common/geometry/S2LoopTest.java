...
public strictfp class S2LoopTest extends GeometryTestCase {
  ...
  public void testBounds() {
    ...
    S2Point mid =
        S2Point.mul(
            S2Point.add(TestLoops.ARCTIC_80.loop.vertex(0), TestLoops.ARCTIC_80.loop.vertex(1)),
            0.5);
    assertDoubleNear(
    ...
    assertBoundsEqual(
        TestLoops.SOUTH_HEMI.loop.getRectBound().lat(), new R1Interval(-S2.M_PI_2, 0), latError);
  }
  ...
  public void testAreaCentroid() {
    ...
    assertDoubleNear(TestLoops.NORTH_HEMI.loop.getArea(), 2 * S2.M_PI);
    assertDoubleNear(TestLoops.EAST_HEMI.loop.getArea(), 2 * S2.M_PI);
  ...
  public void testAreaConsistentWithTurningAngle() {
    ...
    for (TestLoops test : TestLoops.values()) {
      ...
      if (loop.isValid()) {
        double area = loop.getArea();
        double gaussArea = 2 * S2.M_PI - loop.getTurningAngle();
        // TODO(user): The error bound below is much larger than it should be.
        // Need to improve the error minimization analysis in S2.area().
        assertTrue(
            "Failed loop: " + loop + "\nArea = " + area + ", Gauss Area = " + gaussArea,
            Math.abs(area - gaussArea) <= 1e-9);
      }
    }
  }
    ...
    for (int i = 0; i < 50; i++) {
      ...
      do {
        ...
        for (int j = 0; j < numVertices; j++) {
          ...
          vertices.add(S2LatLng.fromRadians(0, uniform(0, S2.M_PI_2)).toPoint());
        }
        loop = new S2Loop(vertices);
      } while (!loop.isValid());
      boolean ccw = loop.isNormalized();
      // TODO(user): The error bound below is much larger than it should be. Need to improve the
      // error minimization analysis in S2::Area().
      assertEquals("Failed loop " + i + ": " + loop, ccw ? 0 : 4 * S2.M_PI, loop.getArea(), 1e-8);
      assertEquals(!ccw, loop.contains(S2Point.Z_POS));
    }
  }

  public void testGetAreaAccuracy() {
    // TODO(user): Test that GetArea() has an accuracy significantly better
  ...
  public void testFourVertexCompressedLoopSize() throws IOException {
    ...
    // 1 byte for originInside and boolean indicating we did not
    //   encode the bound
    // 1 byte for depth
    // Vertices:
    // 1 byte for faces
  ...
  public void testTurningAngle() {
    ...
    assertEquals(2 * S2.M_PI, TestLoops.LINE_TRIANGLE.loop.getTurningAngle(), 1e-14);
    checkTurningAngleInvariants(TestLoops.LINE_TRIANGLE.loop);

    assertEquals(2 * S2.M_PI, TestLoops.SKINNY_CHEVRON.loop.getTurningAngle(), 1e-14);
    ...
    for (int i = 0; i < armVertexCount; i++) {
      ...
      vertices[armVertexCount - i - 1] = S2Point.normalize(new S2Point(r1 * x, r1 * y, 1));
      vertices[armVertexCount + i] = S2Point.normalize(new S2Point(r2 * x, r2 * y, 1));
    }
  ...
  public void testMakeRegularLoop() {
    ...
    assertEquals(103.11051028343408, new S2LatLng(p0).lng().degrees());
