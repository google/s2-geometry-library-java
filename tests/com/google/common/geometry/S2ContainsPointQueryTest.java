...
public class S2ContainsPointQueryTest extends GeometryTestCase {
  ...
  public void testGetContainingShapes() {
    ...
    S2Cap centerCap = S2Cap.fromAxisAngle(randomPoint(), maxLoopRadius);
    S2ShapeIndex index = new S2ShapeIndex();
    for (int i = 0; i < 100; ++i) {
      S2Point center = samplePoint(centerCap);
      S1Angle radius = S1Angle.radians(maxLoopRadius.radians() * rand.nextDouble());
    ...
    for (int i = 0; i < 100; i++) {
      S2Point p = samplePoint(centerCap);
