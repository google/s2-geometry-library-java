...
public strictfp class RealTest extends GeometryTestCase {
  public void benchmark() {
    ...
    while (it.hasNext()) {
      ...
      S2Point b = it.next();
      S2Point c = it.next();
      total += Math.signum(S2Point.crossProd(a, b).dotProd(c));
    }
  ...
  public void testSerialArithmetic() {
    ...
    assertEquals(bigSum.doubleValue(), realSum.doubleValue());
    assertEquals(bigProduct.doubleValue(), realProduct.doubleValue());
  }

  /** Check adding all ranges of values, without risking overflow. */
  public void testSplitArithmetic() {
    double mantissa = Math.PI / 4;
    for (int a = -400; a <= 400; a += 16) {
      for (int b = -400; b <= 400; b += 16) {
        double d1 = Math.scalb(mantissa, a);
        double d2 = Math.scalb(mantissa, b);
  ...
  /**
   * Verifies that native and robust techniques produce nearly equal results all around the sphere.
   */
  public void testRandomCCW() {
    for (int i = 0; i < 1000; i++) {
      ...
      assertEquals("Robust result not close enough to naive result",
          naiveDeterminant, robustDeterminant, 1e-15);
    }
  }
          ...
          for (double x2 = -1; x2 <= 1; x2 += spacing) {
            for (double y2 = -1; x2 <= 1; x2 += spacing) {
              for (double z2 = -1; x2 <= 1; x2 += spacing) {
                // Construct q as the average of p and q. Since p and q are powers of 2 on
                // all coordinates, they're exact, and so is q. 
                ...
                for (S2Point n = S2Point.crossProd(p, q);
                    !S2Point.add(r, n).equals(r); n = S2Point.mul(n, .5)) {
