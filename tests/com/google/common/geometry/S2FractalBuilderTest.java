...
public class S2FractalBuilderTest extends GeometryTestCase {
   ...
   * total length, minimum and maximum radius) and verifies that they are within expected
   * tolerances.  Essentially this directly verifies that the shape constructed *is* a fractal,
   * i.e. the total length of the curve increases exponentially with the level, while the area
   * bounded by the fractal is more or less constant.
   */
  private void assertFractal(int minLevel, int maxLevel, double dimension) {
    ...
    double relativeError = Math.sqrt((numLevels - 1.0) / minVertices);

    // 'expansionFactor' is the total fractal length at level 'n + 1' divided by the total fractal
    // length at level 'n'.
    double expansionFactor = Math.pow(4, 1 - 1 / dimension);
    ...
    // subdivision occurs.
    double trianglePerim = 3 * Math.sqrt(3) * nominalRadius;
    double minLengthSum = trianglePerim * Math.pow(expansionFactor, minLevel);
    ...
    assertDoubleNear(expectedNumVertices, loop.numVertices(),
        relativeError * (expectedNumVertices - minVertices) + 1e-11);

    S2Point center = p;
    double minRadius = 2 * Math.PI;
    double maxRadius = 0;
    double lengthSum = 0;
    ...
    assertDoubleNear(expectedLengthSum, lengthSum,
