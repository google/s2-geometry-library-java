...
public class S2LaxPolygonShapeTest extends GeometryTestCase {
    ...
    for (int i = 0; i < 100; i++) {
      loops.add(
          S2Loop.makeRegularLoop(
              S2LatLng.fromDegrees(0, i).toPoint(),
              S1Angle.degrees(0.1),
              random(3))
          .vertices());
    }
  ...
  public void testCompareToS2Loop() {
    for (int iter = 0; iter < 100; iter++) {
      ...
      fractal.setMaxLevel(random(5));
      fractal.setFractalDimension(uniform(1, 2));
      S2Point center = randomPoint();
      S2Loop loop = fractal.makeLoop(getRandomFrameAt(center), S1Angle.degrees(5));
  ...
  private static void checkEncodeDecode(S2LaxPolygonShape expected) throws IOException {
    for (S2Coder<S2LaxPolygonShape> coder :
        Lists.newArrayList(S2LaxPolygonShape.Coder.FAST, S2LaxPolygonShape.Coder.COMPACT)) {
