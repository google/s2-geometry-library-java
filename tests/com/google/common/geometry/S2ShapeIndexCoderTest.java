...
public class S2ShapeIndexCoderTest extends GeometryTestCase {
  ...
  public void testOverlappingLoops() throws IOException {
    ...
    for (int[] testCase : testCases) {
      ...
      S2ShapeIndex expected = new S2ShapeIndex();
      for (int i = 0; i < numShapes; i++) {
        S2Point center = samplePoint(cap);
        double radiusFraction = rand.nextDouble();
  ...
  public void testSnappedFractalPolylines() throws IOException {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      Matrix3x3 frame = S2.getFrame(makePoint("10:" + i));
  ...
  public void testShapesThreadSafe() throws Exception {
    ...
    for (int i = 0; i < numIterations; i++) {
      ...
      for (int j = 0; j < numShapes; j++) {
        int numVertices = 4 * skewed(10);
        index.add(
            new S2Polygon(S2Loop.makeRegularLoop(randomPoint(), kmToAngle(5), numVertices))
                .shape());
      }
  ...
  public void testIteratorThreadSafe() throws Exception {
    ...
    for (int i = 0; i < numIterations; i++) {
      ...
      int numVertices = 4 * skewed(10);
      index.add(
          new S2Polygon(S2Loop.makeRegularLoop(randomPoint(), kmToAngle(5), numVertices)).shape());
      ByteArrayOutputStream output = new ByteArrayOutputStream();
  ...
  private static S2ShapeIndex decode(byte[] bytes) {
