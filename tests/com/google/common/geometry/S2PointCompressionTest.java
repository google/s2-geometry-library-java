...
public strictfp class S2PointCompressionTest extends GeometryTestCase {
  ...
  // 100 vertex loop near the corner of faces 0, 1, and 2;
  // 15 points snapped to S2CellId.MAX_LEVEL, the others not snapped.
  private List<S2Point> oneHundredVertexMixed15Loop;

  // 100 vertex loop near the corner of faces 0, 1, and 2;
  // 25 points snapped to S2CellId.MAX_LEVEL, the others not snapped.
  ...
  public void setUp() {
    ...
    multiFacePoints.add(S2Point.normalize(S2Projections.faceUvToXyz(0, -0.5, 0.5)));
    multiFacePoints.add(S2Point.normalize(S2Projections.faceUvToXyz(1, -0.5, 0.5)));
    multiFacePoints.add(S2Point.normalize(S2Projections.faceUvToXyz(0, 0.5, -0.5)));
    multiFacePoints.add(S2Point.normalize(S2Projections.faceUvToXyz(2, -0.5, 0.5)));
    multiFacePoints.add(S2Point.normalize(S2Projections.faceUvToXyz(2, 0.5, -0.5)));
    multiFacePoints.add(S2Point.normalize(S2Projections.faceUvToXyz(2, 0.5, 0.5)));
    ...
    for (int i = 0; i < 100; i++) {
      ...
      linePoints.add(S2Point.normalize(S2Projections.faceUvToXyz(0, u, v)));
    }
  ...
  private List<S2Point> makeRegularPoints(int numVertices, double radiusKm, int level) {
    S2Point center = S2Point.normalize(new S2Point(1.0, 1.0, 1.0));
  ...
  private String pointsToString(List<S2Point> points) {
