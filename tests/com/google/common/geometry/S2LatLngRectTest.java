...
public strictfp class S2LatLngRectTest extends GeometryTestCase {
  ...
  public void testGetDirectHausdorffDistancePointToRect() {
    ...
    assertDoubleNear(a1.getDirectedHausdorffDistance(b).radians(), a1.getDistance(b).radians());
    assertDoubleNear(a2.getDirectedHausdorffDistance(b).radians(), a2.getDistance(b).radians());
  ...
  void checkCentroidSplitting(S2LatLngRect r, int splitsLeft) {
    ...
    S2LatLngRect child1;
    if (oneIn(2)) {
      double lat = uniform(r.lat().lo(), r.lat().hi());
      ...
      double lng = uniform(r.lng().lo(), r.lng().hi());
