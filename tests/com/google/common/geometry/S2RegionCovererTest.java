  ...
  public void testRandomCaps() {
    ...
    for (int i = 0; i < 1000; ++i) {
      S2RegionCoverer.Builder covererBuilder =
          S2RegionCoverer.builder().setMaxCells(skewed(10)).setLevelMod(1 + random(3));
      do {
        covererBuilder.setMinLevel(random(kMaxLevel + 1));
        covererBuilder.setMaxLevel(random(kMaxLevel + 1));
  ...
  public void testPolylineCoveringJavaCcConsistency() {
    ...
    List<String> expected =
        Arrays.asList(
            ...
            "6b12ae37cea7",
            "6b12ae37cebb");
    assertEquals(expected, ret.stream().map(m -> m.toToken()).collect(Collectors.toList()));
  }
