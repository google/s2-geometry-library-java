...
public final class BestEdgesTestUtils {
  ...
  static String resultToString(Result<S1ChordAngle> result) {
    return Platform.formatString("distance %s to shape %d edge %d",
        result.distance(), result.shapeId(), result.edgeId());
  }
  ...
  public static boolean checkResultSet(
    ...
    for (Result<S1ChordAngle> yp : y) {
      ...
      long count =
          x.stream()
              .filter(xp -> (xp.edgeId() == yp.edgeId()) && (xp.shapeId() == yp.shapeId()))
              .count();
