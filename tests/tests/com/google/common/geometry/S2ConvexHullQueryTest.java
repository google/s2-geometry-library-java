...
public final class S2ConvexHullQueryTest extends GeometryTestCase {
  ...
  public void testNonConvexPoints() {
    ...
    // between the points. (The four points of a tetrahedron would also work, but this is easier.)
    S2ConvexHullQuery query = new S2ConvexHullQuery();
  ...
  private static void checkNorthPoleLoop(S1Angle radius, int numVertices) {
    // If the radius is very close to 90, then it's hard to predict whether the result will be the
    // full loop or not.
    Preconditions.checkArgument(abs(radius.radians() - PI / 2.0) >= 1e-15);

    S2ConvexHullQuery query = new S2ConvexHullQuery();
    S2Loop loop = S2Loop.makeRegularLoop(new S2Point(0, 0, 1), radius, numVertices);
    query.addLoop(loop);
    S2Loop result = query.getConvexHull();
    if (radius.radians() > PI / 2.0) {
      assertTrue(result.isFull());
    } else {
  ...
  public void testPointsInsideHull() {
    ...
    for (int iter = 0; iter < kIters; ++iter) {
      ...
      // Choose points from within a cap of random size, up to but not including an entire
      // hemisphere.
      S2Cap cap = data.getRandomCap(1e-15, 1.999 * PI);
      S2ConvexHullQuery query = new S2ConvexHullQuery();
      int numPoints1 = data.nextInt(100) + 3;
      for (int i = 0; i < numPoints1; ++i) {
        query.addPoint(data.samplePoint(cap));
      }
      S2Loop hull = query.getConvexHull();

      // When the convex hull is nearly a hemisphere, the algorithm sometimes returns a full cap
      // instead. This is because it first computes a bounding rectangle for all the input points/
      // edges and then converts it to a bounding cap, which sometimes yields a non-convex cap
      // (radius larger than 90 degrees). This should not be a problem in practice (since most
      // convex hulls are not hemispheres), but in order to make this test pass reliably it means
      // that we need to reject convex hulls whose bounding cap (when computed from a bounding
      // rectangle) is not convex.
      //
      // TODO(user): This test can still fail (about 1 iteration in 500,000) because the
      // S2LatLngRect.getCapBound implementation does not guarantee that A.Contains(B) implies
      ...
      for (int i = 0; i < numPoints2; ++i) {
        S2Point p = data.samplePoint(cap);
        if (hull.contains(p)) {
          query.addPoint(p);
        }
      }
