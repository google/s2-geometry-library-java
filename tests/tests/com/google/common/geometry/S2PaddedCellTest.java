...
public class S2PaddedCellTest extends GeometryTestCase {
  ...
  public void testS2CellMethods() {
    ...
    for (int iter = 0; iter < iters; ++iter) {
      ...
      double padding = pow(1e-15, data.nextDouble());
  ...
  public void testShrinkToFit() {
    ...
    for (int iter = 0; iter < iters; ++iter) {
      ...
      S2CellId result = data.getRandomCellId();
      ...
      double maxPadding = 0.5 * min(uvSize.x, uvSize.y);
      double padding = maxPadding * data.nextDouble();
      ...
      if (!result.isLeaf()) {
        ...
        Axis axis = data.nextInt(2) == 0 ? Axis.X : Axis.Y;
