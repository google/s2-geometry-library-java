...
public class S2ClosestPointQueryTest extends GeometryTestCase {
  ...
  private static void checkResultSet(
    ...
    // Result set X should contain all the items from U whose distance is less than "limit" computed
    // below.
    double limit = 0;
    if (x.size() < maxSize) {
      // Result set X was not limited by "maxSize", so it should contain all the items up to
      // "maxDistance", except that a few items right near the distance limit may be missed because
      // the distance measurements used for pruning S2Cells are not conservative.
      limit = maxDistance.radians() - maxPruningError.radians();
    } else if (!x.isEmpty()) {
      // Result set X contains only the closest "maxSize" items, to within a tolerance of
  ...
  protected enum PointFactory {
    /**
     * Generator for points regularly spaced along a circle. The circle is centered within the
     * query cap and occupies 25% of its area, so that random query points have a 25% chance of
          ...
          for (int j = 0; j < sqrtNumPoints; ++j) {
            points.add(
                S2.fromFrame(
                    frame,
                    new S2Point(
                        tan((i + 0.5) * spacing - radius),
                        tan((j + 0.5) * spacing - radius),
                        1.0).normalize()));
          }
        }
        return points;
  ...
  private void checkFactory(PointFactory factory, int numIndexes, int numPoints, int numQueries) {
    ...
    for (int i = 0; i < numIndexes; i++) {
      ...
      index.reset();
      addPoints(index, factory.createPoints(data, queryCap, numPoints));
      query.reset();
      for (int j = 0; j < numQueries; j++) {
        query.setMaxPoints((int) data.uniform(1, 100));
        if (data.oneIn(2)) {
          query.setMaxDistance(randomAngle());
        }
        S2LatLngRect rect =
            S2LatLngRect.fromCenterSize(
                new S2LatLng(data.samplePoint(queryCap)),
                new S2LatLng(randomAngle(), randomAngle()));
        if (data.oneIn(5)) {
        ...
        if (data.oneIn(2)) {
          ...
          S2Point b = data.samplePoint(S2Cap.fromAxisAngle(a, randomDecimilliAngle()));
  ...
  protected S1Angle randomDecimilliAngle() {
    return S1Angle.radians(pow(1e-4, randomAngle().radians()));
  }

  private S1Angle randomAngle() {
    return S1Angle.radians(data.uniform(0, QUERY_RADIUS.radians()));
  }
}
