...
public class S2Test extends GeometryTestCase {
  ...
  /**
   * Verifies that the machine epsilon is exactly the same as the value in {@link S2#DBL_EPSILON}.
   */
  @Test
  public void testMachineEpsilon() {
    ...
    assertExactly(S2.DBL_EPSILON, machEps);
  }
  ...
  public void testSTUV() {
    ...
    for (double x = 0; x <= 1; x++) {
      assertDoubleNear(S2Projections.stToUV(x), 2 * x - 1);
      assertDoubleNear(S2Projections.uvToST(2 * x - 1), x);
    }
    ...
    for (double x = 0; x <= 1; x += 0.0001) {
      ...
      assertDoubleNear(S2Projections.stToUV(S2Projections.uvToST(2 * x - 1)), 2 * x - 1);
    }
  }
  ...
  public void testXYZToFaceSiTi() {
    ...
    for (int level = 0; level <= S2CellId.MAX_LEVEL; level++) {
      for (int i = 0; i < 1000; ++i) {
        ...
        FaceSiTi result = S2Projections.xyzToFaceSiTi(p);
        int resultLevel = S2Projections.levelIfCenter(result, p);
        ...
        FaceSiTi moved = S2Projections.xyzToFaceSiTi(pMoved);
        assertEquals(-1, S2Projections.levelIfCenter(moved, pMoved));
        ...
        S2Point pRandom = S2Projections.faceSiTiToXyz(faceRandom, siRandom, tiRandom);
        FaceSiTi actual = S2Projections.xyzToFaceSiTi(pRandom);
        int actualLevel = S2Projections.levelIfCenter(actual, pRandom);
  ...
  public void testAngleArea() {
    ...
    assertAlmostEquals(S2.angle(p000, pz, p045), M_PI_4);
    assertEquals(S2.turnAngle(p000, pz, p045), -3 * M_PI_4, 0);

    assertAlmostEquals(S2.angle(p045, pz, p180), 3 * M_PI_4);
    assertAlmostEquals(S2.angle(p000, pz, p180), PI);

    assertDoubleNear(S2.angle(pz, p000, pz), 0);
    assertAlmostEquals(S2.angle(pz, p000, p045), M_PI_2);

    assertAlmostEquals(S2.area(p000, p090, pz), M_PI_2);
    assertAlmostEquals(S2.area(p045, pz, p180), 3 * M_PI_4);
    ...
    assertAlmostEquals(quarterArea1, PI);
    ...
    assertAlmostEquals(quarterArea2, PI);
  ...
  public void testMetrics() {
    MetricBundle angleSpan =
        new MetricBundle(MIN_ANGLE_SPAN, MAX_ANGLE_SPAN, AVG_ANGLE_SPAN);
    MetricBundle width = new MetricBundle(MIN_WIDTH, MAX_WIDTH, AVG_WIDTH);
    MetricBundle edge = new MetricBundle(MIN_EDGE, MAX_EDGE, AVG_EDGE);
    MetricBundle diag = new MetricBundle(MIN_DIAG, MAX_DIAG, AVG_DIAG);
    MetricBundle area = new MetricBundle(MIN_AREA, MAX_AREA, AVG_AREA);
    ...
    assertTrue(MAX_EDGE_ASPECT >= 1.0);
    assertTrue(MAX_EDGE.deriv() / MIN_EDGE.deriv() >= MAX_EDGE_ASPECT);
    assertTrue(MAX_DIAG_ASPECT >= 1);
    assertTrue(MAX_DIAG.deriv() / MIN_DIAG.deriv() >= MAX_DIAG_ASPECT);
    ...
    assertTrue(MIN_AREA.deriv() >= MIN_WIDTH.deriv() * MIN_EDGE.deriv() - 1e-15);
    assertTrue(MAX_AREA.deriv() <= MAX_WIDTH.deriv() * MAX_EDGE.deriv() + 1e-15);
    ...
    assertEquals(S2CellId.MAX_LEVEL, MIN_WIDTH.getMinLevel(0));
    assertEquals(S2CellId.MAX_LEVEL, MAX_WIDTH.getMinLevel(0));
    ...
    assertEquals(0, MIN_WIDTH.getMaxLevel(4));
    assertEquals(0, MAX_WIDTH.getMaxLevel(4));
    ...
    for (int level = -2; level <= S2CellId.MAX_LEVEL + 3; ++level) {
      double dWidth = scalb(MIN_WIDTH.deriv(), -level);
      // Check lengths.
      ...
      assertEquals(expectedLevel, MIN_WIDTH.getMinLevel(dWidth));
      assertEquals(expectedLevel, MIN_WIDTH.getMaxLevel(dWidth));
      assertEquals(expectedLevel, MIN_WIDTH.getClosestLevel(dWidth));

      // Also check non-boundary cases.
      assertEquals(expectedLevel, MIN_WIDTH.getMinLevel(1.2 * dWidth));
      assertEquals(expectedLevel, MIN_WIDTH.getMaxLevel(0.8 * dWidth));
      assertEquals(expectedLevel, MIN_WIDTH.getClosestLevel(1.2 * dWidth));
      assertEquals(expectedLevel, MIN_WIDTH.getClosestLevel(0.8 * dWidth));

      // Check areas.
      double dArea = MIN_AREA.deriv() * pow(4, -level);
      if (level <= -3) {
        dArea = 0;
      }
      assertEquals(expectedLevel, MIN_AREA.getMinLevel(dArea));
      assertEquals(expectedLevel, MIN_AREA.getMaxLevel(dArea));
      assertEquals(expectedLevel, MIN_AREA.getClosestLevel(dArea));
      assertEquals(expectedLevel, MIN_AREA.getMinLevel(1.2 * dArea));
      assertEquals(expectedLevel, MIN_AREA.getMaxLevel(0.8 * dArea));
      assertEquals(expectedLevel, MIN_AREA.getClosestLevel(1.2 * dArea));
      assertEquals(expectedLevel, MIN_AREA.getClosestLevel(0.8 * dArea));
    }
  }
  ...
  public void testTrueCentroidForSmallTriangle() {
    ...
    // S2Polygon.contains(S2Point) requires a normalized point, but the centroid is not normalized.
  ...
  public void testTrueCentroidSemiEquator() {
    ...
    assertAlmostEquals(2.0, centroid.norm());
  }
