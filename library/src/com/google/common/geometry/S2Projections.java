...
public strictfp enum S2Projections {
  ...
  public static int stToIj(double s) {
    return max(0, min(S2CellId.MAX_SIZE - 1, (int) round(S2CellId.MAX_SIZE * s - 0.5)));
  }
  ...
  static XyzTransform faceToXyzTransform(int face) {
    ...
    //
    // TODO(user): Remove this legacy behavior.
  ...
  int levelIfCenter(FaceSiTi fst, S2Point p) {
    ...
    if (level < 0 || level != siTiToLevel(fst.ti)) {
      ...
      S2Point center = faceSiTiToXyz(fst.face, fst.si, fst.ti).normalize();
      if (p.equals(center)) {
        return level;
      } else {
        return -1;
