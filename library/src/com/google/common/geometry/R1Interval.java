...
public final class R1Interval implements Serializable {
  ...
  /** Interval constructor. If {@code lo > hi}, the interval is empty. */
  ...
  /** Returns an empty interval. (Any interval where {@code lo > hi} is considered empty.) */
  ...
  enum Endpoint {
    ...
    /** Returns the value of the interval at this endpoint. */
    public abstract double getValue(R1Interval interval);

    /** Sets the value of the interval at this endpoint. */
  ...
  /**
   * Return an interval that has been expanded on each side by the given distance "margin". If
  ...
  public R1Interval union(R1Interval y) {
    ...
    return new R1Interval(min(lo, y.lo), max(hi, y.hi));
  }
  ...
  public boolean approxEquals(R1Interval y, double maxError) {
    ...
    return abs(y.lo - lo) <= maxError && abs(y.hi - hi) <= maxError;
  }
